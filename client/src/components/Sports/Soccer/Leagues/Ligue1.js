...
const Ligue1 = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/ligue-1-data");
										...
										<td>
											<input
												...
												checked={myGames.some(
													(game) => game.id === `L1${fixture.MatchNumber}`
												)}
												onChange={() => {
													const game = {
														...fixture,
														MatchNumber: "L1" + fixture.MatchNumber,
														id: `L1${fixture.MatchNumber}`,
													};
