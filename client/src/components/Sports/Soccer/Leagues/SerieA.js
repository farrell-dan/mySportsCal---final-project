...
const SerieA = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/serie-a-data");
										...
										<td>
											<input
												...
												checked={myGames.some(
													(game) => game.id === `SA${fixture.MatchNumber}`
												)}
												onChange={() => {
													const game = {
														...fixture,
														MatchNumber: "SA" + fixture.MatchNumber,
														id: `SA${fixture.MatchNumber}`,
													};
