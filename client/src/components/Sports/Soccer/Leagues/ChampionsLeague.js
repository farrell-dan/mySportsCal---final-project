...
const ChampionsLeague = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/champions-league-data");
					...
					<table className="fixture-table">
						...
						<tbody>
							{upcomingGames.slice(0, visibleGames).map((fixture, index) => {
								...
								return (
									<tr key={"UCL" + fixture.MatchNumber}>
										...
										<td>
											<input
												...
												checked={myGames.some(
													(game) => game.id === `UCL${fixture.MatchNumber}`
												)}
												onChange={() => {
													const game = {
														...fixture,
														MatchNumber: "UCL" + fixture.MatchNumber,
														id: `UCL${fixture.MatchNumber}`,
													};
