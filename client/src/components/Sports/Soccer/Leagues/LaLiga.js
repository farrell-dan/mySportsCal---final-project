...
const LaLiga = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				const response = await fetch("http://localhost:3000/api/laliga-data");
						...
						<table className="fixture-table">
							...
							<tbody>
								{upcomingGames.slice(0, visibleGames).map((fixture, index) => {
									...
									return (
										<tr key={"LaLiga" + fixture.MatchNumber}>
											...
											<td>
												<input
													...
													checked={myGames.some(
														(game) => game.id === `LaLiga${fixture.MatchNumber}`
													)}
													onChange={() => {
														const game = {
															...fixture,
															MatchNumber: "LaLiga" + fixture.MatchNumber,
															id: `LaLiga${fixture.MatchNumber}`,
														};
